...
impl VhdFileSystem {
    ...
    pub fn file_entry_exists(&self, path: &Path) -> bool {
        ...
        match path.get_component_by_index(1) {
            Some(path_component) => {
                ...
                let layer_index: usize = match VfsPath::get_numeric_suffix(path_component, "vhd") {
    ...
    pub fn get_file_entry_by_path(&self, path: &Path) -> Result<Option<VhdFileEntry>, ErrorTrace> {
        if path.is_relative() {
            return Ok(None);
        }
        match path.get_component_by_index(1) {
    ...
    pub fn get_root_file_entry(&self) -> VhdFileEntry {
        VhdFileEntry::Root {
            image: self.image.clone(),
        }
    }
