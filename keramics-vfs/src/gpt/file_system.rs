...
impl GptFileSystem {
        ...
        match path.get_component_by_index(1) {
            Some(path_component) => {
                ...
                let partition_index: usize =
                    match VfsPath::get_numeric_suffix(path_component, "gpt") {
    ...
    pub fn get_file_entry_by_path(&self, path: &Path) -> Result<Option<GptFileEntry>, ErrorTrace> {
        if path.is_relative() {
            return Ok(None);
        }
        match path.get_component_by_index(1) {
    ...
    pub fn get_root_file_entry(&self) -> GptFileEntry {
        GptFileEntry::Root {
    ...
    /// Opens a GPT volume system.
    pub(crate) fn open_volume_system(
        volume_system: &mut GptVolumeSystem,
