...
pub struct VmdkImageLayer {
    ...
    /// Parent layer.
    parent_layer: Option<Arc<RwLock<VmdkImageLayer>>>,
...
impl VmdkImageLayer {
            ...
            let range_read_count: usize = match &extent.extent_type {
                ...
                | VmdkDescriptorExtentType::VmfsSparse => {
                    ...
                    match self.extent_file_cache.get_mut(&lookup_extent_index) {
                        ...
                        Some(VmdkExtentFile::SparseCowd(_sparse_file)) => {
                        ...
                        Some(VmdkExtentFile::SparseVmdk(sparse_file)) => {
                            ...
                            match block_range.range_type {
                                ...
                                VmdkBlockRangeType::InParentOrSparse => {
                                    match self.parent_layer.as_ref() {
                                        Some(parent_layer) => {
                                            keramics_core::data_stream_read_at_position!(
                                                parent_layer,
                                                &mut data[data_offset..data_end_offset],
    ...
    pub fn set_parent(
        &mut self,
        parent_layer: &Arc<RwLock<VmdkImageLayer>>,
    ) -> Result<(), ErrorTrace> {
        ...
        match parent_layer.read() {
            Ok(image_layer) => {
                if *parent_content_identifier != image_layer.content_identifier {
                    return Err(keramics_core::error_trace_new!(format!(
                        "Parent content identifier: {} does not match content identifier of parent layer: {}",
                        parent_content_identifier, image_layer.content_identifier,
                    )));
                }
            }
            Err(error) => {
...
impl DataStream for VmdkImageLayer {
...
mod tests {
    ...
    // TODO: add tests for get_parent_file_name
    ...
    // TODO: add tests for read_data_from_extents
    ...
    fn test_get_offset() -> Result<(), ErrorTrace> {
        let mut image_layer: VmdkImageLayer = get_image_layer()?;
